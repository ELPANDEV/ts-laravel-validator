...
permitted_values.forEach(({ value, rule_value }) => {
  test(`validator_required_if(${value}, ${rule_value}) to equal true`, () => {
    expect(validator_required_if(value, rule_value, values)).toEqual(true);
  })
})
...
non_permitted_values.forEach(({ value, rule_value }) => {
  test(`validator_required_if(${value}, ${rule_value}) to equal false`, () => {
    expect(validator_required_if(value, rule_value, values)).toEqual(false);
  })
})
