...
permitted_values.forEach(({ value, rule_value }) => {
  test(`validator_gt(${value}, ${rule_value}) to equal true`, () => {
    expect(validator_gt(value, rule_value, values)).toEqual(true);
  })
})
...
non_permitted_values.forEach(({ value, rule_value }) => {
  test(`validator_gt(${value}, ${rule_value}) to equal false`, () => {
    expect(validator_gt(value, rule_value, values)).toEqual(false);
  })
})
